...
| `destination_account_id` | Receiving account ID stored as bigint snapshot value (no foreign key constraint) |
...
This means the `payments` table keeps a foreign-key relationship to `users.id` through `source_account_id`, while `destination_account_id` stores the receiving account ID directly as bigint without a foreign key constraint.
...
|---|---|---|---:|---|---|---|---|
| 101 | 1001 | 6214830000000001 | 1500.00 | USD | SENT | 2026-07-27 10:30:00 | 2026-07-27 10:35:00 |
...
| `destination_account_id` | 收款账户ID（BIGINT 快照值，不设置外键约束） |
| `amount` | 支付金额 |
| `currency` | 支付所使用的货币 |
| `status` | 支付当前状态 |
| `created_at` | 支付创建时间 |
...
这意味着 `payments` 表通过 `source_account_id` 与 `users.id` 建立外键关联，而 `destination_account_id` 以 BIGINT 直接保存收款账户ID，不设置外键约束。
