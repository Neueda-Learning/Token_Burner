package com.example.paymentprocessing.service;

/**
 * TODO Richard:
 * Define the service contract for payment use cases.
 * Add method signatures for creation, retrieval, history lookup, and status updates later.
 *
 * Future business responsibilities:
 * - Validate the payment request.
 * - Verify the payment password before creating a payment.
 * - Compare the user input password with the stored payment password hash.
 * - Reject invalid payment passwords.
 * - Save payment status history after successful status changes.
 *
 * Do not implement these responsibilities at this stage.
 */
public interface PaymentService {
    // TODO Add payment service method declarations.

}

